...
from ipatests.test_xmlrpc.xmlrpc_test import fuzzy_certfieldmappingrule
...
class CertprofileTracker(Tracker):
    ...
    retrieve_all_keys = retrieve_keys | {'objectclass', 'ipacertfieldmapping'}
    create_keys = retrieve_keys | {'objectclass'}
    update_keys = retrieve_keys - {'dn'}
    managedby_keys = retrieve_keys
    allowedto_keys = retrieve_keys
    # Keys that are passed to Tracker.update() but never returned in the response
    ...
    def track_create(self):
        ...
        if self.mappings:
            self.attrs['ipacertfieldmapping'] = [fuzzy_certfieldmappingrule for rule in self.mappings]
    ...
    def update(self, updates, expected_updates=None, mappings=None):
        ...
        if mappings is not None:
            ...
            self.attrs['ipacertfieldmapping'] = [fuzzy_certfieldmappingrule for rule in mappings]

        return super(CertprofileTracker, self).update(passed_updates, expected_updates)
